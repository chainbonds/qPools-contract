...
export class MarinadeState {
  ...
  static async fetch (marinade: Marinade) { // @todo rework args
    ...
    return new MarinadeState(marinade, marinade.provider, state);
  }
  ...
  mSolMint = MarinadeMint.build(this.provider, this.mSolMintAddress)
  ...
  lpMint = MarinadeMint.build(this.provider, this.lpMintAddress)
