...
use crate::state::{PortfolioAccount, PositionAccountLarix, UserCurrencyAccount};
...
pub struct ApprovePositionWeightLarix<'info> {
    ...
    #[account(
        init,
        payer = owner,
        space =8 + PositionAccountLarix::LEN,
        seeds = [
            owner.key().as_ref(),
            &_index.to_le_bytes(),
...
pub fn handler(
    ctx: Context<ApprovePositionWeightLarix>,
    _bump_portfolio: u8,
    _bump_position: u8,
    _bump_currency: u8,
    _weight: u64,
